...
class GymnasiumWrapper(dm_env.Environment):
    def __init__(self, env, seed):
        ...
        self._action_spec = specs.BoundedArray(shape=action_space.shape,
                                               dtype=np.float32,
        ...
        self._obs_spec = specs.Array(shape=flat_obs.shape,
                                     dtype=np.float32,
                                     name='observation')
        self._latest_obs = flat_obs
...
class ObservationDTypeWrapper(dm_env.Environment):
    def __init__(self, env, dtype):
        ...
        self._dtype = dtype
        wrapped_obs_spec = env.observation_spec()
        if isinstance(wrapped_obs_spec, dict):
            wrapped_obs_spec = wrapped_obs_spec['observations']
...
def _make_ant(obs_type, action_repeat, seed):
    ...
    try:
        ...
        raise ImportError('gymnasium-robotics is required for Ant. '
